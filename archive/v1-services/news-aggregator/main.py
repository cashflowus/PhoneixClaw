...
async def _run_aggregator(service):
    global _healthy
    for attempt in itertools.count(1):
        try:
            await service.start()
            _healthy = True
            logger.info("News aggregator started (attempt %d)", attempt)
            await service.run()
            break
...
async def lifespan(app: FastAPI):
    from services.news_aggregator.src.service import NewsAggregatorService

    service = NewsAggregatorService()
    task = asyncio.create_task(_run_aggregator(service))
