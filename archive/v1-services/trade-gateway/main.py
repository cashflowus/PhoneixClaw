...
async def _run_gateway(service):
    global _healthy
    for attempt in itertools.count(1):
        try:
            await service.start()
            _healthy = True
            logger.info("Trade gateway connected to Kafka (attempt %d)", attempt)
            await service.run()
            break
            ...
            logger.exception(
                "Trade gateway startup failed (attempt %d), retrying in %ds",
                attempt, delay,
            )
...
async def lifespan(app: FastAPI):
    ...
    service = TradeGatewayService()
    task = asyncio.create_task(_run_gateway(service))
