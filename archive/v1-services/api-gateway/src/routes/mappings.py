...
async def list_mappings(
    ...
    account_ids = await _user_account_ids(request.state.user_id, session)
    if not account_ids:
        return []
    result = await session.execute(
        select(AccountSourceMapping).where(
            AccountSourceMapping.trading_account_id.in_(account_ids)
        )
    )
...
async def create_mapping(
    req: MappingCreate, request: Request,
    session: AsyncSession = Depends(get_session),
):
    account_ids = await _user_account_ids(request.state.user_id, session)
    if uuid.UUID(req.trading_account_id) not in account_ids:
        raise HTTPException(
            status_code=403, detail="Trading account does not belong to you"
        )
...
async def delete_mapping(
    ...
    result = await session.execute(
        select(AccountSourceMapping).where(
            AccountSourceMapping.id == uuid.UUID(mapping_id),
            AccountSourceMapping.trading_account_id.in_(account_ids),
        )
    )
...
def _mapping_response(m: AccountSourceMapping) -> MappingResponse:
    return MappingResponse(
        id=str(m.id),
        trading_account_id=str(m.trading_account_id),
        channel_id=str(m.channel_id),
        config_overrides=m.config_overrides or {},
        enabled=m.enabled,
    )
