...
SERVICE_NAME = "ai-trade-recommender"
...
async def _run_recommender(service):
    global _healthy
    for attempt in itertools.count(1):
        try:
            await service.start()
            _healthy = True
            logger.info("AI Trade Recommender connected (attempt %d)", attempt)
            await service.run()
            break
...
async def lifespan(app: FastAPI):
    from services.ai_trade_recommender.src.service import AITradeRecommenderService

    service = AITradeRecommenderService()
    task = asyncio.create_task(_run_recommender(service))
